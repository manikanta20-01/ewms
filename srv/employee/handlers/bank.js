...
module.exports = async (srv) => {
  ...
  srv.before("CREATE", "Banks", async (req) => {
    ...
    if (!bankName) req.error(400, "Bank Name is required.");

    if (!accountHolderName) req.error(400, "Account Holder Name is required.");

    if (!accountNumber) req.error(400, "Account Number is required.");

    if (!ifscCode) req.error(400, "IFSC Code is required.");
    ...
    if (duplicate) req.error(400, "Bank account already exists.");
  ...
  srv.before("UPDATE", "Banks", async (req) => {
    ...
    if (!currentBank) req.error(404, "Bank account not found.");
