...
module.exports = async (srv) => {
  ...
  const { Employee, Experience } = cds.entities("ewms.db.employee");
  ...
  srv.before("CREATE", Experience, async (req) => {
    ...
    if (!companyName) req.error(400, "Company Name is required.");

    if (!designation) req.error(400, "Designation is required.");

    if (!joiningDate) req.error(400, "Joining Date is required.");
    ...
    if (employee) req.error(400, "Active Employee not found.");
    ...
    const duplicate = await tx.run(
      SELECT.one.from(Experience).where({
        employee_ID,
        companyName,
        designation,
        joiningDate,
    ...
    if (duplicate) req.error(400, "Experience already exists.");
  });
